...
ALTER TABLE interest_cycles DROP CONSTRAINT IF EXISTS interest_cycles_status_check;

-- Add the new constraint with UPPERCASE statuses
ALTER TABLE interest_cycles 
ADD CONSTRAINT interest_cycles_status_check 
CHECK (status IN ('PENDING', 'PAID', 'OVERDUE', 'PARTIAL', 'OPEN'));

-- Update existing lowercase statuses to uppercase if any (just in case)
UPDATE interest_cycles SET status = UPPER(status);

-- Update functions to use UPPERCASE statuses
